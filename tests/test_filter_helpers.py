...
class TestReadAnnoData:
    ...
    def test_read_anno_data_returns_dataframe_on_valid_csv(self, tmp_path):
        ...
        df = filters._read_anno_data(anno_path, None)
    ...
    def test_read_anno_data_raises_csvreaderror_on_bad_input(self, tmp_path):
        ...
        with pytest.raises(flask_utils.CSVReadError) as excinfo:
            filters._read_anno_data(anno_path, None)
...
class TestFilterDf:
    ...
    def test_filter_df_filters_selected_ids(self, tmp_path, app_filter):
        ...
        with app_filter.test_request_context('/fake', method='POST', data=form_data):
    ...
    def test_filter_df_no_selected_ids_returns_full_df(self, tmp_path, app_filter):
        # Create dummy pandas dataframe
        df = pd.DataFrame({
            'Patient_ID': ['A', 'B', 'C'],
            'Value': [10, 20, 30]
...
class TestShowFilterPage:
    ...
    def test_show_filter_page_renders_page(self, app_filter):
        ...
        with app_filter.test_request_context('/fake'):
            html = filters._show_filter_page(df, filtered_df, selected_ids, applied_text)
        ...
        # Page should contain checkboxes or elements referencing all ids 1,2,3
        assert '1' in html
        assert '2' in html
        assert '3' in html